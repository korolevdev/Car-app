...
class BT(object):
    ...
    def recv_packet(self, timeout=None):
        ...
        while timeout is None or time.time() < t0 + timeout:
            if timeout is not None: self.ser.timeout = t0 + timeout - time.time() # Could be bug?
            c = self.ser.read()
            if not c: return None # If c is none, return none.
...
class MyoRaw(object):
    ...
    def connect(self):
        ...
        if self.old:
            ...
            #print("%%%%: ", pack('BBBBHBBBBB', 2, 9, 2, 1, C, emg_smooth, C // emg_hz, imu_hz, 0, 0))
            ...
            ## enable IMU data
            #print("IMU data starts")
            self.write_attr(0x1d, b'\x01\x00')
            #print("IMU data starts")
            ## enable on/off arm notifications
            #print("Arm notifications start")
            self.write_attr(0x24, b'\x02\x00')
            #print("Arm notifications end")
    ...
    def start_raw(self):
        ...
        pose notifications.
        '''
        #print("Start raw starts")
        self.write_attr(0x28, b'\x01\x00')
        self.write_attr(0x19, b'\x01\x03\x01\x01\x00')
        self.write_attr(0x19, b'\x01\x03\x01\x01\x01')
        #print("Start raw ends")
